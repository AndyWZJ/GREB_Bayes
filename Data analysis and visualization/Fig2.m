...
nexttile
load('E:\GREB\data\Accuracy\Natural_breakpoint_atsurf_5class')
[x,y,z,v] = flow;
...
nexttile
load('E:\GREB\data\Accuracy\Natural_breakpoint_atsurf_7class')
[x,y,z,v] = flow;
sx = linspace(min(min(min(x)))+1.5,max(max(max(x))),5);
sy = [1,-1]; sz = [1,-1];
slice(x,y,z,v,sx,sy,sz);
...
nexttile
load('E:\GREB\data\Accuracy\Natural_breakpoint_atsurf_9class')
[x,y,z,v] = flow;
sx = linspace(min(min(min(x)))+1.5,max(max(max(x))),5);
sy = [1,-1]; sz = [1,-1];
slice(x,y,z,v,sx,sy,sz);
